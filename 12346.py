...
def getTbody(pattern, table):
    ...
    # print table
    count_num_lst = map(lambda x: ('0', '0', x[2], x[2]), lst[:-1])
    # print count_num_lst
...
def sub_all_table(content):
    ...
    table_lst = demotestRe(tables, content)
    table_tbody_sub_pattern = re.compile(
        '<tr class=".*?">\s+<td>.*?</td>.*?<td.*?>(\d+)</td>.*?<td.*?>(\d+)</td>.*?<td.*?>(\d+)</td>.*?<td.*?>(\d+)</td>.*?</tr>',
        re.S)
    new_tables = []
    ...
    vuln_risk_category_pattern = re.compile(
        u'<div class="report_h report_h2" id="vuln_risk_category">2.1.*?<div class="report_h report_h2"', re.S)

    # print vuln_risk_category_pattern.search(content).group()
    content = vuln_risk_category_pattern.sub(tables_html.encode('utf-8'), content)
    # print content
    return content
...
def subHighPic(content):
    ...
    content = re.sub(hight_pateern, safe_img, content)
    # print content[1]
